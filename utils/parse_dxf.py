...
def dxf2img(dxf, nestID=None, reverse=False, mirror=False):
    ...
    if reverse:
        ...
        for k in list(textdb.keys()):
            for i in range(len(textdb[k])):
                textdb[k][i] = (np.round(resize[0] - textdb[k][i][0]).astype(int), textdb[k][i][1])
...
def segmentPieces(img, textDB, textSZ, num_expected):
    ...
    # print("LCH:: Max level is ", max_level)

    # while max_level > 1:
    for max_level in [11, 7, 3]:
        ...
        for i in range(len(contours)):
            if outer_level_target(i, hierarchy, max_level-3):
                temp_local = temp.copy()
                inverse_local = np.zeros_like(temp_local)
                cv2.drawContours(canvas, contours, i, 255, thickness)
                cv2.drawContours(temp, contours, i, 255, thickness)
                cv2.drawContours(temp_local, contours, i, 255, thickness)
                cv2.drawContours(inverse_local, contours, i, 255, -1)
                cv2.drawContours(inverse, contours, i, 255, -1)
                temp_local = cv2.bitwise_and(temp_local, inverse_local)
                rect = cv2.boundingRect(contours[i])
                piece = temp_local[rect[1]: rect[1] + rect[3], rect[0]: rect[0] + rect[2]].copy()
...
def parse_dxf_by_info(dxf, partInfo, nestID, reverse=False, mirror=False):
    '''
    reverse=True代表翻转180度，reverse=False代表不翻转
    mirror=True代表水平镜像，mirror=False代表不镜像
    reverse和mirror不可同时为True
    :param dxf:
    :param partInfo:
    :param nestID:
    :return:
    '''
    img, textDB, textSZ, num = dxf2img(dxf, nestID, reverse=reverse, mirror=mirror)
...
if __name__ == "__main__":
    ...
    # dxf = "data/dxf/O210811SG100015A05.dxf"
    # dxf = "data/dxf/O210929SG10008A01.dxf"  # level 3
    # dxf = "data/dxf/O210919SG100010A02.dxf"  # level 9
    # dxf = "data/dxf/O211005SG10005A03M.dxf"
    # dxf = "data/dxf/O211006SG10005A12.dxf"
    dxf = "data/dxf/O211007SG10006A22.dxf"

    img, textDB, textSZ, num = dxf2img(dxf, reverse=False, mirror=False)
    partDB = segmentPieces(img, textDB, textSZ, num)
    print(textDB)
