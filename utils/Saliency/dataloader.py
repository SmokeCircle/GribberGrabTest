...
class SODLoader(Dataset):
    ...
    def __init__(self, mode='train', augment_data=False, target_size=256):
        if mode == 'train':
            self.inp_path = './data/workpieces/img'
            self.out_path = './data/workpieces/mask'
        elif mode == 'test':
            self.inp_path = './data/workpieces/test/img'
            self.out_path = './data/workpieces/test/mask'
        else:
...
class InfDataloader(Dataset):
    ...
    def __init__(self, img_folder, target_size=256):
        ...
        if os.path.isdir(self.imgs_folder):
            print("image path is directory")
            self.img_paths = sorted(glob.glob(self.imgs_folder + '/*'))
        else:
            print("image path is an image")
            self.img_paths = [self.imgs_folder]
        self.target_size = target_size
        self.normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                              std=[0.229, 0.224, 0.225])
    ...
    def __getitem__(self, idx):
        ...
        rotate: a flag that indicates whether the input image has been rotated.
        """
        img = cv2.imread(self.img_paths[idx])
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        if img.shape[0] < img.shape[1]:  # if H < W, it's a lie-down image
