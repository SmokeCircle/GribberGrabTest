...
public:
    ...
    std::vector<Match> match(cv::Mat sources, float threshold,
                                                     ...
                                                     const cv::Mat masks = cv::Mat(), bool debug = false) const;
...
public:
    ...
    void produce_infos(){
        ...
        if(angle_range.size() == 1 && scale_range.size() == 1){
        ...
        }else if(angle_range.size() == 2 && scale_range.size() == 2 && angle_step != 90){
            assert(scale_range[1] > scale_range[0]);
            assert(angle_range[1] > angle_range[0]);
            for(float scale = scale_range[0]; scale <= scale_range[1]+eps; scale += scale_step){
                for(float angle = angle_range[0]; angle <= angle_range[1]+eps; angle += angle_step){
            ...
            for(float scale = scale_range[0]; scale <= scale_range[1]+eps; scale += scale_step){
                for(float angle = angle_range[0]; angle <= angle_range[1]+eps; angle += angle_step){
                    for(float glitch = -5.0; glitch <= 5.0+eps; glitch += 1){
                        if (angle + glitch < 0) infos.emplace_back(angle+glitch+360, scale);
                        else if (angle + glitch > 360) infos.emplace_back(angle+glitch-360, scale);
                        else infos.emplace_back(angle+glitch, scale);
                    }                    
                }
            }
        }
